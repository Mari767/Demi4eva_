...
void MASTER::setName(string name) {
	m_name = name; 
}
void MASTER::setColvo_dogs(int colvo_dogs) {
	m_colvo_dogs = colvo_dogs;
}
...
int MASTER::getColvo_dogs() {
	return m_colvo_dogs; 
}
int MASTER::getCount() { 
	int count = m_count; return count;
}
DOG MASTER::getDog_of_master(int number) { 
	return m_dog_of_master[number];
}
string MASTER::getDog_nickname() {
	return m_dog_of_master->getNickname(); 
}
int MASTER::getDog_weight() { 
	return m_dog_of_master->getWeight();
}
int MASTER::getDog_age() { 
	return m_dog_of_master->getAge(); 
}
